...
def version_matches_range(version, version_range):
    ...
    operator, range_version = re.match("([<>=!]*)(\d.*)", version_range).groups()
...
def normalize_version_to_semver(version):
    ...
    initial, dev = re.match("(.*?)(\.dev.*)?$", version).groups()
    ...
    numeric, after = re.match("(\d+\.\d+\.\d+)([^\d].*)?", version).groups()
    ...
    after_pieces = re.match("([a-z])(\d+)", after)
    if after_pieces:
