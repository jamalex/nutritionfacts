...
class PythonVersionFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        matching = [ver for ver in pyvers if ver.startswith(prefix) if ver.count(".") == target_comp_count]
...
class PingbackAdmin(ReadOnlyAdmin):
    ...
    list_filter = (DefaultModeFilter, ('saved_at', DateRangeFilter), 'kolibri_version', 'mode')
...
class InstanceAdmin(ReadOnlyAdmin):
    ...
    list_filter = (DefaultModeFilter, ('first_seen', DateRangeFilter), ('last_seen', DateRangeFilter), PythonVersionFilter, 'last_mode')
